import {
  store,
  APP_CONFIG, AUTH_COPY, ONBOARDING_TOUR, FLASHCARDS, MISSIONS, BADGES, LEADERBOARD,
  SETTINGS_OPTIONS, MOOD_OPTIONS, COURSES, MOCK_DAYS, FREE_TIER_MAX_DAY,
  getCourseDay, daysBetween, isSameWeek, persistProgress, practiceQuestions,
} from "./session.js";
import { handleTypedSubmit } from "./voice.js";
...
export function route() {
  ...
  if (store.tab === "home") { showScreen("home"); setOfflineBanner(); renderHome(); }
...
export function renderHome() {
  ...
  const locked = user.plan === "free" && store.progress.courseDay > FREE_TIER_MAX_DAY;
  let weeklyDays = 0;
  if (isSameWeek(store.progress.weeklyStartDay, store.progress.simDateOffset)) {
    const weekStart = store.progress.weeklyStartDay;
    weeklyDays = store.progress.practiceDays.filter((d) => daysBetween(weekStart, d) >= 0 && daysBetween(weekStart, d) < 7).length;
  }
  ...
  $("home-scroll").innerHTML = `
    <div class="flex items-center justify-between px-5 pt-6 pb-3">
      ...
      <button data-act="rewards" class="flex items-center gap-1.5 rounded-full bg-accent-500/10 px-3 py-1.5">
        ${svg("flame", 16)}
...
export function renderCoursePlan() {
  ...
  $("courseplan-weeks").innerHTML = course.weeks.map((week) => {
    ...
    const days = expanded ? week.days.map((day) => {
      ...
      const locked = store.auth.user.plan === "free" && day.day > FREE_TIER_MAX_DAY;
...
export function renderRewards() {
  ...
  const practicedSet = new Set(store.progress.practiceDays);
  const monthDays = [];
  for (let d = 1; d <= daysInMonth; d++) {
    const dateStr = `${year}-${String(month + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
    monthDays.push({ date: dateStr, day: d, practiced: practicedSet.has(dateStr) });
  }
  ...
  $("rewards-scroll").innerHTML = `
    ...
    <div class="mt-5 px-5">
      ...
      <div class="rounded-2xl bg-surface-0 p-4 shadow-soft">
        <div class="grid grid-cols-7 gap-1.5">${monthDays.map((d) => `<div class="flex aspect-square items-center justify-center rounded-lg text-xs font-bold transition-all ${d.practiced ? "bg-accent-500 text-white" : "bg-surface-2 text-ink-300"}">${d.day}</div>`).join("")}</div>
  ...
  $("profile-scroll").innerHTML = `
    ...
    <div class="mt-5 px-5">
      ...
      ${stats.every((s) => s.value === 0)
        ? `<div class="flex items-center gap-3 rounded-2xl bg-surface-0 p-4 shadow-soft"><div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-brand-500/10 text-brand-500">${svg("mic", 20)}</div><p class="text-sm text-ink-500">Finish your first mission to start tracking store.progress</p></div>`
    ...
    <div class="mt-5 px-5 space-y-2.5">
      <button data-act="manage-plan" class="flex w-full items-center gap-3 rounded-2xl bg-surface-0 p-4 text-left shadow-soft transition-transform active:scale-[0.98]">
      ...
      <button data-act="my-data" class="flex w-full items-center gap-3 rounded-2xl bg-surface-0 p-4 text-left shadow-soft">
        <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-ink-100 text-ink-500">${svg("trash", 20)}</div>
        <div class="flex-1"><p class="text-sm font-bold text-ink-900">My data</p><p class="text-xs text-ink-400">Delete my recordings</p></div>${svg("chevronRight", 18)}
      </button>
    ...
    ${APP_CONFIG.DEMO_MODE ? `<div class="mt-5 px-5"><div class="rounded-2xl border-2 border-dashed border-ink-200 bg-surface-2/50 p-4"><p class="mb-2 text-xs font-bold uppercase tracking-wide text-ink-400">Demo tools</p><button data-act="advance-day" class="flex w-full items-center gap-3 rounded-xl bg-surface-0 p-3 text-left shadow-soft transition-transform active:scale-95">${svg("fastForward", 18)}<div class="flex-1"><p class="text-sm font-bold text-ink-900">Skip to next day</p><p class="text-xs text-ink-400">Currently: Day ${store.progress.courseDay} · Offset: +${store.progress.simDateOffset}d</p></div></button></div></div>` : ""}
    <p class="mt-6 text-center text-xs text-ink-300">Byo<span class="text-accent-500 font-bold">U</span> v0.2 · Prototype</p>`;
}
...
export function renderPractice() {
  ...
  if (!store.practice.feedback) {
    ...
    if (isRecording) {
      live += `<div class="mb-3 flex items-center justify-between"><div class="flex items-center gap-1.5"><div class="flex items-center gap-0.5 h-5">${[0,1,2,3,4].map((i) => `<span class="w-1 rounded-full bg-accent-500 animate-wave" style="animation-delay:${i * 100}ms;height:100%"></span>`).join("")}</div><span class="ml-1.5 text-xs font-semibold text-accent-500">Recording</span></div><span class="rounded-md bg-ink-100 px-2 py-0.5 text-xs font-mono font-semibold text-ink-600">${formatTime(store.practice.elapsedSec)}</span></div>`;
    }
    if (hasAnyText || isRecording) {
      live += `<p class="text-sm leading-relaxed text-ink-700">${store.practice.transcript}${store.practice.transcript && store.practice.liveText ? " " : ""}<span class="text-ink-400">${store.practice.liveText}</span>${isRecording && !store.practice.transcript && !store.practice.liveText ? `<span class="text-ink-400">Listening...</span>` : ""}${isRecording ? `<span class="ml-0.5 inline-block h-4 w-0.5 animate-pulse bg-brand-500 align-middle"></span>` : ""}</p>`;
  ...
  if (store.practice.feedback) {
    fb = `<div class="mt-5 space-y-3 animate-slide-up">
      ...
      <div class="rounded-2xl bg-surface-0 p-4 shadow-soft"><div class="flex items-center gap-2"><div class="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-50 text-brand-500">${svg("repeat", 16)}</div><p class="text-sm font-bold text-ink-900">Say it once more</p></div><p class="mt-2 text-sm leading-relaxed text-ink-600">${store.practice.feedback[2].body}</p>
        <button data-act="rerecord" class="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-accent-500/10 px-3 py-2 text-sm font-medium text-accent-500 transition-transform active:scale-95">${svg("rotate", 15)} Re-record (10s)</button>
      </div>
  ...
  $("practice-scroll").innerHTML = `
    <div class="flex gap-2.5 animate-fade-in">
      ...
      <div class="flex-1 rounded-2xl rounded-tl-md bg-surface-0 px-4 py-3 shadow-soft">
        <p class="text-sm font-medium leading-relaxed text-ink-800">${prompt.text}</p>
...
export function renderFlashcard() {
  if (store.flash.done) {
    $("flashcard-main").innerHTML = `
      <div class="flex flex-1 flex-col items-center justify-center px-6 text-center animate-fade-in">
        <div class="flex h-20 w-20 items-center justify-center rounded-full bg-success-500/10 text-success-500 animate-scale-in">${svg("layers", 36)}</div>
        <h2 class="mt-6 text-xl font-extrabold text-ink-900">Deck complete!</h2>
        <p class="mt-2 text-sm text-ink-500">You learned ${store.flash.learned} of ${FLASHCARDS.length} words</p>
        <button data-act="claim-cards" class="tap-target inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-200 bg-brand-500 text-white shadow-pop hover:bg-brand-600 active:scale-[0.98] w-full mt-8 max-w-[260px]">Claim +${store.flash.learned * 2} points ${svg("chevronRight", 16)}</button>
      </div>`;
    return;
  }
  const card = FLASHCARDS[store.flash.idx];
  ...
  $("flashcard-main").innerHTML = `
    <div class="flex items-center justify-between px-5 pt-5 pb-3">
      ...
      <div class="flex items-center gap-1.5">${FLASHCARDS.map((_, i) => `<div class="h-1.5 rounded-full transition-all ${i === store.flash.idx ? "w-6 bg-brand-500" : i < store.flash.idx ? "w-4 bg-brand-300" : "w-1.5 bg-ink-200"}"></div>`).join("")}</div>
      <div class="w-9"></div>
    </div>
    <div class="flex flex-1 flex-col items-center justify-center px-6">
      <div class="relative w-full max-w-[300px]" style="transform:${transform};opacity:${store.flash.exitDir ? 0 : 1};transition:${store.flash.exitDir ? "all 0.25s ease-out" : "all 0.3s ease-out"}">
        <button data-act="flip-card" class="relative flex h-72 w-full flex-col items-center justify-center rounded-3xl bg-surface-0 p-6 text-center shadow-card">
          ${!store.flash.flipped
            ? `<p class="text-xs font-semibold uppercase tracking-wide text-ink-400">Word ${store.flash.idx + 1} of ${FLASHCARDS.length}</p><h2 class="mt-3 text-3xl font-extrabold text-ink-900">${card.word}</h2><button data-act="say-word" class="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-brand-500/10 px-3 py-2 text-sm font-medium text-brand-500">${svg("volume", 16)} Say it</button><p class="mt-4 text-xs text-ink-400">Tap to flip</p>`
            : `<div class="animate-fade-in"><p class="text-sm font-bold text-ink-900">${card.meaning}</p><p class="mt-3 rounded-xl bg-surface-2 px-4 py-3 text-sm italic text-ink-600">"${card.example}"</p><p class="mt-4 text-xs text-ink-400">Tap to flip back</p></div>`}
        </button>
      </div>
      <div class="mt-8 flex items-center gap-4">
        <button data-act="swipe-left" class="tap-target flex h-12 w-12 items-center justify-center rounded-full bg-surface-0 text-ink-400 shadow-soft transition-transform active:scale-90">${svg("x", 22)}</button>
        <button data-act="swipe-right" class="tap-target flex h-14 w-14 items-center justify-center rounded-full bg-success-500 text-white shadow-popSuccess transition-transform active:scale-90">${svg("check", 26)}</button>
      </div>
      <p class="mt-4 text-xs font-medium text-ink-400">${store.flash.learned} learned · swipe or tap ✓</p>
    </div>`;
}
...
export function handleSwipe(dir) {
  ...
  setTimeout(() => {
    ...
    if (store.flash.idx < FLASHCARDS.length - 1) { store.flash.idx += 1; store.flash.flipped = false; }
    else store.flash.done = true;
    renderFlashcard();
  }, 250);
}
...
export function renderDeleteSheet() {
  $("delete-done-msg").classList.toggle("hidden", !store.deleteConfirmed);
  $("delete-actions").innerHTML = !store.deleteConfirmed
    ? `<button data-act="confirm-delete" class="tap-target inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-200 bg-accent-500 text-white shadow-soft hover:bg-accent-600 active:scale-[0.98] w-full">Yes, delete recordings</button>
       <button data-act="cancel-delete" class="tap-target inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-200 text-ink-600 hover:bg-surface-2 active:scale-[0.98] w-full">Cancel</button>`
    : `<button data-act="done-delete" class="tap-target inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold transition-all duration-200 bg-brand-500 text-white shadow-pop hover:bg-brand-600 active:scale-[0.98] w-full">Done</button>`;
}

export function updateSignupSubmit() {
  const name = $("signup-name").value.trim();
  const email = $("signup-email").value.trim();
  const pw = $("signup-pw").value;
  const pwValid = pw.length >= 8;
  const can = name && email && pwValid && store.consentVoice && store.consentTerms;
  $("signup-submit").disabled = !can;
  $("signup-fill-hint").classList.toggle("hidden", !(!can && $("signup-helper").classList.contains("hidden")));
  const hint = $("signup-pw-hint");
  if (pw.length === 0) { hint.textContent = ""; }
  else if (pwValid) { hint.textContent = AUTH_COPY.signup.ok; hint.className = "mt-1.5 block text-xs text-success-600"; }
  else { hint.textContent = AUTH_COPY.signup.weak; hint.className = "mt-1.5 block text-xs text-accent-500"; }
}

export function setCheck(btn, on) {
  btn.className = `mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 transition-all ${on ? "border-brand-500 bg-brand-500" : "border-ink-300 bg-surface-0"}`;
  btn.innerHTML = on ? `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><path d="M20 6 9 17l-5-5"/></svg>` : "";
}
